#!/usr/bin/env python3
"""
Elemetris Persona Bot — with RAG knowledge base and admin panel.
...
CONFIG = {
    "persona_name": os.environ.get("PERSONA_NAME", "Elemetris Advisor"),
    "opening": os.environ.get(
        "PERSONA_OPENING",
        "Hello, welcome to your session with the Elemetris Advisor.",
    ),
    ...
    "footer_cta_text": os.environ.get(
        "FOOTER_CTA_TEXT",
        "To schedule time with an Elemetris Advisor, please",
    ),
...
# Restricted names
# ---------------------------------------------------------------------------
# Internal / legal-entity names the bot must never surface. Empty by default.
# Add Elemetris's restricted names here (and they will be both instructed
# against in the system prompt and scrubbed from any model output as backup),
# e.g. RESTRICTED_NAMES = ["Some Legal Entity LLC", "Internal Product Name"].
# You can also drive this from an env var if you prefer:
#   RESTRICTED_NAMES = [n.strip() for n in
#       os.environ.get("RESTRICTED_NAMES", "").split(",") if n.strip()]
RESTRICTED_NAMES = []
...
<head>
  ...
  <style>
    :root {
      ...
      --text: #101418;
      --rust: #C0392B;
      --shadow: 0 1px 2px rgba(16, 20, 24, 0.05), 0 8px 28px rgba(16, 20, 24, 0.07);
    }
    * { box-sizing: border-box; }
    ...
    .brand-divider { width: 1px; height: 38px; background: rgba(0, 116, 248, 0.35); }
    .brand-tag {
    ...
    header button {
      ...
      border: 1px solid rgba(0, 116, 248, 0.35);
    ...
    header button:hover {
      background: rgba(0, 116, 248, 0.08);
    ...
    input[type="text"]:focus { border-color: var(--gold); box-shadow: 0 0 0 3px rgba(0, 116, 248, 0.18); }
    .mic-btn {
...
def chat():
    ...
    scope_guard = (
        ...
        "1. You answer ONLY questions related to Elemetris's areas of expertise: "
        ...
        "where Elemetris would have no specific expertise — you MUST politely decline "
        "and redirect.\n\n"
        "3. Your off-topic decline should be brief and warm, in the Elemetris voice. "
        "Use this format (adapt naturally):\n"
        "   \"That's outside what I'm here to help with as the Elemetris Advisor. "
        ...
        "4. Do NOT attempt to bridge an off-topic question into Elemetris territory. "
        ...
        "A. NEVER use any restricted internal or legal-entity names in your "
        "responses. (Add Elemetris's specific restricted names to the list "
        "below if applicable.)\n"
        + (
            "".join(f"   - \"{n}\"\n" for n in RESTRICTED_NAMES)
            + "   - any variation, partial form, hyphenation, abbreviation, or "
              "rephrasing of those names\n\n"
            if RESTRICTED_NAMES else "\n"
        )
        + "B. Refer to the organization only as \"Elemetris\" if you must "
        "mention it by name. Otherwise, prefer phrases like \"our approach,\" "
        "\"our frameworks,\" \"the methodology,\" or simply describe the "
        "concept directly without attribution.\n\n"
        "C. If the user explicitly asks about a restricted name, respond in a "
        "way that discusses the underlying ideas, tools, or frameworks WITHOUT "
        "naming those specific brands. Do not confirm or deny that those names "
        "exist. Pivot to the substance.\n\n"
        "D. If retrieved context from the knowledge base contains any "
        "restricted name, paraphrase the content so the restricted names do "
        "NOT appear in your response. The underlying ideas can be conveyed "
        "without the trademarked names.\n\n"
    ...
    if context:
        composed_prompt = (
            base_prompt
            + "\n\n---\nRELEVANT CONTEXT FROM ELEMETRIS KNOWLEDGE BASE:\n\n"
            + context
    ...
    # Defensive scrubber: replace any restricted brand names if the model slips them through.
    # The system prompt instructs Claude not to use these, but we sanitize as backup.
    # Empty by default — populate RESTRICTED_NAMES (top of file) with Elemetris's
    # restricted names. Each restricted name is replaced with a neutral phrase.
    FORBIDDEN_NAMES = [(n, "the organization") for n in RESTRICTED_NAMES]
    import re as _re
...
def admin_export_feedback():
    ...
    filename = f"elemetris_feedback_{timestamp}.csv"
...
def admin_export_feedback_xlsx():
    ...
    header_font = Font(bold=True, color="0074F8", size=11)
    header_fill = PatternFill("solid", fgColor="101418")
    header_align = Alignment(horizontal="left", vertical="center", wrap_text=False)
    thin_border = Border(
        bottom=Side(style="medium", color="101418"),
    )
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"elemetris_feedback_{timestamp}.xlsx"
